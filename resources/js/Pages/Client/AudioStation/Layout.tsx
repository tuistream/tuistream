...
import {
    ...
    Info, Settings, FolderOpen, ListMusic, Volume2, Calendar,
    Link2, Globe, Disc, Users, Music, FileText, BarChart3, Lock, Trash2, Menu, X
} from 'lucide-react';
...
const menuItems = [
    ...
    { label: 'Archivos', icon: FolderOpen, route: 'media', slug: '/media' },
    { label: 'Listas de reproducción', icon: ListMusic, route: 'playlists', slug: '/playlists' },
    { label: 'Jingles', icon: Volume2, route: 'jingles', slug: '/jingles' },
    { label: 'Calendario', icon: Calendar, route: 'schedule', slug: '/schedule' },
    { label: 'Widgets y enlaces', icon: Link2, route: 'widgets', slug: '/widgets' },
    { label: 'Página pública', icon: Globe, route: 'public', slug: '/public' },
    { label: 'Puntos de montaje', icon: Disc, route: 'mountpoints', slug: '/mount-points' },
    { label: 'Gerente de D.J.', icon: Users, route: 'djs', slug: '/djs' },
    { label: 'Título de la canción', icon: Music, route: 'songtitle', slug: '/song-title' },
    { label: 'Archivos de registro', icon: FileText, route: 'logs', slug: '/logs' },
    { label: 'Informes', icon: BarChart3, route: 'reports', slug: '/reports' },
];

export default function AudioStationLayout({ children, currentSection = 'show' }: {
    ...
    const handleRestart = () => {
        if (confirm('¿Reiniciar los servicios de streaming de audio?')) {
            ...
            <aside className="w-64 shrink-0 border-r border-slate-900 bg-slate-950/50 backdrop-blur-sm hidden md:flex flex-col h-screen sticky top-0 z-50">
                    ...
                    <div className="flex items-center justify-center mb-4 border-b border-slate-900/30 pb-3 select-none">
                        {app?.logo ? (
                            ...
                            <div className="flex items-center gap-2.5 w-full">
                                <div className="w-7 h-7 rounded-lg bg-indigo-500 flex items-center justify-center text-white font-black text-xs shadow-lg shadow-indigo-500/20">
                                    TS
                                </div>
                ...
                <nav className="flex-1 overflow-y-auto p-3 space-y-0.5">
                    ...
                    <div className="pt-3 border-t border-slate-900/50 mt-3 space-y-0.5">
                        <Link
                            href={`/dashboard/station/${station.id}/suspend`}
                            className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-semibold transition-all ${
                        ...
                        <Link
                            href={`/dashboard/station/${station.id}/delete`}
                            className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-semibold transition-all ${
                ...
                <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-45 md:hidden flex justify-end">
                    <div className="w-80 h-full bg-slate-950 border-l border-slate-900 flex flex-col p-6 animate-[slideIn_0.3s_ease-out] overflow-y-auto">
                        <div className="flex items-center justify-between pb-4 border-b border-slate-900">
                            <span className="font-bold text-xs text-indigo-400 uppercase tracking-wider">Menú Estación</span>
                            <button
