...
export default function VideoStationSuspend() {
    const { station } = usePage<any>().props as unknown as PageProps;
    const { post, processing } = useForm({});

    const handleSuspend = (e: React.FormEvent) => {
        e.preventDefault();
        post(`/dashboard/canaltv/${station.id}/suspend`);
    };

    return (
        <VideoStationLayout currentSection="suspend">
                        ...
                        {station.is_active ? (
                            <>
                                ...
                                <ul className="list-disc pl-5 space-y-1 text-slate-500">
                                    <li>La transmisión en vivo se desconectará inmediatamente.</li>
                                    <li>Los reproductores insertados en páginas web externas mostrarán un estado de desconexión.</li>
                                    <li>Su ingesta RTMP no aceptará nuevas conexiones.</li>
                                    <li><strong>No se perderán sus archivos ni configuraciones guardadas.</strong> Podrá reactivarlo en cualquier momento.</li>
                                </ul>
                            </>
                        ) : (
                            <p>Su canal de video se encuentra actualmente suspendido. Reactívelo para aceptar conexiones de ingesta RTMP de inmediato.</p>
                        )}
                    </div>
