...
export default function VideoStationLayout({ children, currentSection = 'show' }: {
    ...
    const handleRestart = () => {
        if (confirm('¿Reiniciar los servicios de streaming de video?')) {
    ...
    return (
        <div className="min-h-screen bg-slate-950 text-slate-100 font-sans flex flex-col md:flex-row overflow-x-hidden relative">
            {/* Glowing background */}
            <div className="fixed top-0 right-1/4 w-150 h-150 bg-pink-500/5 rounded-full blur-3xl pointer-events-none" />
            ...
            <header className="md:hidden h-16 border-b border-slate-900 bg-slate-950/80 backdrop-blur-md sticky top-0 z-40 px-4 flex items-center justify-between">
                <div className="flex items-center gap-2">
                    {app?.logo ? (
                        ...
                        <>
                            <div className="p-2 bg-gradient-to-br from-pink-500 to-violet-600 rounded-lg text-white">
            ...
            {/* Desktop Sidebar (Hidden on Desktop) */}
            <aside className="w-64 shrink-0 border-r border-slate-900 bg-slate-950/50 backdrop-blur-sm hidden md:flex flex-col h-screen sticky top-0 z-50">
                    ...
                    <div className="flex items-center justify-center mb-4 border-b border-slate-900/30 pb-3 select-none">
                        {app?.logo ? (
                            ...
                            <div className="flex items-center gap-2.5 w-full">
                                <div className="w-7 h-7 rounded-lg bg-pink-500 flex items-center justify-center text-white font-black text-xs shadow-lg shadow-pink-500/20">
                                    TS
                                </div>
                    ...
                    <div className="flex items-center gap-3">
                        <div className={`p-2 rounded-lg border ${
                            station.status === 'online'
                                ...
                                : 'bg-slate-955 border-slate-900 text-slate-500'
                        }`}>
                ...
                <nav className="flex-1 overflow-y-auto p-3 space-y-0.5">
                    {menuItems.map((item) => {
                        ...
                        const href = `/dashboard/canaltv/${station.id}${item.slug}`;
                        return (
                            <Link
                                key={item.label}
                                href={href}
                    ...
                    <div className="pt-3 border-t border-slate-900/50 mt-3 space-y-0.5">
                        <Link
                            href={`/dashboard/canaltv/${station.id}/suspend`}
                            className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-semibold transition-all ${
                                currentSection === 'suspend'
                        ...
                        <Link
                            href={`/dashboard/canaltv/${station.id}/delete`}
                            className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-semibold transition-all ${
                ...
                <div className="p-4 border-t border-slate-900 shrink-0">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="w-7 h-7 rounded-full bg-linear-to-br from-pink-500 to-violet-600 flex items-center justify-center text-white text-[10px] font-bold">
                            {auth?.user?.name?.charAt(0)?.toUpperCase() || 'C'}
                        </div>
                        <div className="min-w-0 flex-1">
                            <Link href="/dashboard/profile" className="text-xs font-semibold text-slate-300 hover:text-pink-400 transition-colors truncate block">{auth?.user?.name}</Link>
                ...
                <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-45 md:hidden flex justify-end">
                    <div className="w-80 h-full bg-slate-950 border-l border-slate-900 flex flex-col p-6 animate-[slideIn_0.3s_ease-out] overflow-y-auto">
                        <div className="flex items-center justify-between pb-4 border-b border-slate-900">
                            <span className="font-bold text-xs text-pink-400 uppercase tracking-wider">Menú Canal TV</span>
                            <button
                        ...
                        <div className="pt-4 border-t border-slate-900 space-y-3 shrink-0">
                            ...
                            <div className="flex items-center gap-3">
                                <div className="w-9 h-9 rounded-full bg-pink-500/10 flex items-center justify-center text-pink-400 text-xs font-bold">
                ...
                {menuItems.map((item) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            {isActive && (
                                <span className="absolute bottom-0 w-4 h-0.5 rounded-full bg-pink-500 animate-pulse" />
                            )}
                        </Link>
                    );
                })}
