...
export default function VideoStationDelete() {
    const { station } = usePage<any>().props as unknown as PageProps;
    const { delete: destroy, processing } = useForm({});

    const handleDelete = (e: React.FormEvent) => {
        e.preventDefault();
        if (confirm(`¿Está COMPLETAMENTE seguro de eliminar permanentemente el canal "${station.name}"? Esta acción no se puede deshacer.`)) {
            destroy(`/dashboard/canaltv/${station.id}/delete`);
        }
    };

    return (
        <VideoStationLayout currentSection="delete">
            ...
            <div className="max-w-2xl mx-auto mt-8">
                <div className="rounded-2xl border border-red-500/20 bg-red-500/5 backdrop-blur-sm overflow-hidden p-6 space-y-6">
                    ...
                    <div className="text-xs text-slate-400 leading-relaxed space-y-3">
                        ...
                        <p>Al eliminar de forma permanente este servicio de streaming de video:</p>
                        <ul className="list-disc pl-5 space-y-1 text-slate-500 font-medium">
                            <li>Se apagarán todos los contenedores y puertos de ingesta.</li>
                            <li>Toda la configuración del Canal de TV se borrará permanentemente.</li>
                            <li>Se destruirán de forma definitiva todos los archivos almacenados.</li>
