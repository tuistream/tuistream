...
import {
    Video, ArrowLeft, Save, User, Server, Zap, HardDrive,
    Cast, Lock, Globe, RefreshCw, Plus, X, Tv, Radio, Play
} from 'lucide-react';
...
interface StationData {
    ...
    transcoder_profiles: string[];
    stream_targets_limit: number;
...
export default function VideoStationEdit() {
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        stream_targets_count:  station.stream_targets_limit > 0 ? station.stream_targets_limit : 0,
        selected_platforms:    station.selected_platforms || [] as string[],
        geoip_locking:         station.geoip_locking || false,
        ndvr_rewind:           station.ndvr_rewind || false,
    });
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(`/admin/video/${station.id}/edit`);
    };
                        ...
                        <div>
                            ...
                            <div className="flex gap-3 mb-3">
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="disk_space" checked={data.disk_space_limit === -1}
                                        onChange={() => setData('disk_space_limit', -1)} className="accent-pink-500" />
                                    <span className="text-xs text-slate-300">Unlimited</span>
                                </label>
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="disk_space" checked={data.disk_space_limit !== -1}
                                        onChange={() => setData('disk_space_limit', data.disk_space_mb)} className="accent-pink-500" />
                            ...
                            <div className="flex gap-3 mb-3">
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="data_transfer" checked={data.data_transfer_limit === -1}
                                        onChange={() => setData('data_transfer_limit', -1)} className="accent-pink-500" />
                                    <span className="text-xs text-slate-300">Unlimited</span>
                                </label>
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="data_transfer" checked={data.data_transfer_limit !== -1}
                                        onChange={() => setData('data_transfer_limit', data.data_transfer_mb)} className="accent-pink-500" />
                                    <span className="text-xs text-slate-300">Personalizado</span>
                                </label>
                            </div>
                ...
                <div className="rounded-2xl border border-slate-900 bg-slate-900/20 backdrop-blur-sm p-6 relative overflow-hidden">
                    ...
                    <div className="space-y-6">
                        ...
                        <div>
                            ...
                            <div className="flex gap-3 mb-3">
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="stream_targets" checked={data.stream_targets_limit === -1}
                                        onChange={() => setData('stream_targets_limit', -1)} className="accent-pink-500" />
                                    <span className="text-xs text-slate-300">Unlimited</span>
                                </label>
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="stream_targets" checked={data.stream_targets_limit !== -1}
                                        onChange={() => setData('stream_targets_limit', data.stream_targets_count)} className="accent-pink-500" />
                            ...
                            {data.stream_targets_limit !== -1 && (
                                <div className="mb-4">
                                    ...
                                    <input type="range" min={0} max={50} value={data.stream_targets_count}
                                        onChange={e => {
                        ...
                        {/* Toggles */}
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div onClick={() => setData('geoip_locking', !data.geoip_locking)}
                            ...
                            >
                                <div className={`w-9 h-5 rounded-full relative transition-all duration-300 flex-shrink-0 ${data.geoip_locking ? 'bg-pink-500' : 'bg-slate-700'}`}>
                                    <div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-all duration-300 ${data.geoip_locking ? 'left-4' : 'left-0.5'}`} />
                                </div>
                                <div>
                            ...
                            <div onClick={() => setData('ndvr_rewind', !data.ndvr_rewind)}
                                className={`flex items-center gap-3 p-4 rounded-xl border cursor-pointer transition-all select-none ${
                                    data.ndvr_rewind
                                        ? 'border-pink-500/40 bg-pink-500/10'
                                        : 'border-slate-800 bg-slate-950/60 hover:border-slate-700'
                            ...
                            >
                                <div className={`w-9 h-5 rounded-full relative transition-all duration-300 flex-shrink-0 ${data.ndvr_rewind ? 'bg-pink-500' : 'bg-slate-700'}`}>
                                    <div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-all duration-300 ${data.ndvr_rewind ? 'left-4' : 'left-0.5'}`} />
                                </div>
                                <div>
                                    <p className="text-xs font-bold text-slate-300 flex items-center gap-1.5">
